...
function Conversion() {
    ...
    const [selectedUnitData, setSelectedUnitData] = useState<StorageUnitData>()

    const [measurements, setMeasurementTypes] = useState<string[]>(['Decimal'])
    const [unitList, setUnitList] = useState<StorageUnitData[]>([]);
    ...
    useEffect(() => {
        setUnitList(storageUnitList.filter((data) => data.type == 'Base' || measurements.includes(data.type)))
    }, [measurements])
    ...
    useEffect(() => {
        ...
        if (!currentUnitData) {
            showToast('The current storage unit is not supported now.', 'danger', 3000)
            return;
        }
        setSelectedUnitData(currentUnitData);
    ...
    return (
        <section id="conversion">
            <div className="mt-4">
                <label htmlFor="currentInput" className="form-label h5 text-primary fw-bolder ">{selectedUnitData?.title} Conversion</label>
                <input type="number" className="form-control form-control-lg" id="currentInput" min={0} placeholder="" value={current} onChange={(e) => {
                    setCurrent(parseFloat(e.target.value))
                }} />
            </div>
            <div className="row">
                <div className="col-12 col-lg-6 mt-4">
                    <select className="form-select form-select-lg" value={selectedUnit} aria-label="Storage Unit" onChange={(e) => {
                        setSelectedUnit(e.target.value)
                    }}>
                        {
                            unitList.map((data) => {
                                return <option key={data.unit} value={data.unit}>{data.title}</option>
                            })
                        }
                    </select>
                </div>
                <div className="col-12 col-lg-6 mt-4">
                    <div className="d-flex justify-content-start align-items-center">
                        <div className="form-check col-auto form-control-lg">
                            <input className="form-check-input" type="checkbox" value="Decimal" id="decimalCheck" checked={measurements.includes('Decimal')} onChange={toggleMeasurementTypes} />
                            <label className="form-check-label" htmlFor="decimalCheck">
                                Decimal
                            </label>
                        </div>
                        <div className="form-check col-auto form-control-lg">
                            <input className="form-check-input" type="checkbox" value="Binary" id="binaryCheck" checked={measurements.includes('Binary')} onChange={toggleMeasurementTypes} />
                            <label className="form-check-label" htmlFor="binaryCheck">
                                Binary
                            </label>
                        </div>
                        <div className="form-check col-auto form-control-lg">
                            <input className="form-check-input" type="checkbox" value="Bit" id="bitCheck" checked={measurements.includes('Bit')} onChange={toggleMeasurementTypes} />
                    ...
                    <tbody className="table-group-divider">
                        {
                            outputs.map((op, index) => {
                                return (
                                    <tr key={op.unit.unit}>
                                        <th scope="row">{op.unit.title} (<span className="text-danger">{op.unit.unit}</span>)</th>
                                        <td  >
                                            {op.valueView}
            ...
            <tbody>
                {
                    list.map((cnv, index) => {
                        ...
                        return (
                            <tr key={index}>
                                <th scope="row">1 <span className="text-primary">{cnv.from}</span></th>
...
function StorageUnitPage({ toolData, relatedTools }: InferGetStaticPropsType<typeof getStaticProps>) {
    return (
        <>
            ...
            <Layout title={toolData.title} relatedTools={relatedTools}>
                <div className="container pt-3">
                    <Conversion />
                    <div className="text-center h5 mt-4 text-uppercase">
